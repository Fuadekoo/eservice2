...
/**
 * Role Permission Assignment Utility
 * 
...
/**
 * Assign default permissions to a role by role ID (fetches role name from database)
 * @param roleId - The role ID
 * @returns Object with success status and assigned permissions count
 */
export async function assignDefaultPermissionsToRoleById(
  roleId: string
): Promise<{ success: boolean; assignedCount: number; error?: string }> {
  try {
    ...
    return await assignDefaultPermissionsToRole(roleId, role.name);
  } catch (error: any) {
