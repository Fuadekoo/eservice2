...
interface CustomerRequestStore {
  ...
  uploadFile: (requestId: string, file: File, description?: string) => Promise<void>;
...
export const useCustomerRequestStore = create<CustomerRequestStore>((set, get) => ({
  // Initial state
  requests: [],
  services: [],
  ...
  fetchRequestById: async (id: string) => {
    try {
      set({ isLoading: true });
      const response = await fetch(`/api/request/${id}`, { cache: "no-store" });
  ...
  setSelectedRequest: (request: Request | null) => set({ selectedRequest: request }),
}));
